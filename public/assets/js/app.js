...
// Simple language switching with page refresh
document.addEventListener("DOMContentLoaded", function() {
    ...
    const languageDisplays = document.querySelectorAll('.language-display, .d-none.d-sm-inline');
    languageDisplays.forEach(display => {
        if (display.textContent.length <= 3) {
            display.textContent = currentLocale.toUpperCase();
        }
    });
});
